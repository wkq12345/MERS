...
<div class="container py-4">
    <h1 class="mb-4"><i class="bi bi-pencil-fill me-2"></i>Edit Tourist Spot</h1>

    <div class="card shadow-sm">
        <div class="card-body">
            <form action="{{ route('super-admin.tourist_spots.update', $spot) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                           name="name" value="{{ old('name', $spot->name) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description"
                              name="description" rows="4">{{ old('description', $spot->description) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('location_id') is-invalid @enderror" id="location_id" name="location_id">
                        ...
                        @foreach ($locations as $loc)
                            <option value="{{ $loc->id }}" {{ old('location_id', $spot->location_id) == $loc->id ? 'selected' : '' }}>
                                {{ $loc->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-check form-switch mb-3">
                    <input class="form-check-input" type="checkbox" role="switch" id="status" name="status" value="1"
                           {{ old('status', $spot->status) ? 'checked' : '' }}>
                    <label class="form-check-label" for="status">Active</label>
                </div>

                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        <label for="image" class="form-label">Upload New Image (jpg, png, webp)</label>
                        ...
                        @if ($spot->image)
                            <small class="text-muted d-block mt-2">
                                Current image:
                                <a href="{{ preg_match('/^https?:\/\//', $spot->image) ? $spot->image : asset($spot->image) }}" target="_blank">View</a>
                            </small>
                        @endif
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="url" class="form-control @error('image_url') is-invalid @enderror" id="image_url"
                               name="image_url"
                               value="{{ old('image_url', preg_match('/^https?:\/\//', $spot->image) ? $spot->image : '') }}"
                                ...
                                @if ($type->criteria->count())
                                    <div class="row g-3">
                                        @foreach ($type->criteria as $criterion)
                                            @php $current = old('ratings.' . $criterion->id, $ratings[$criterion->id] ?? null); @endphp
                                            <div class="col-md-6">
