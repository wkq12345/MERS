...
@section('content')
    @php
        ...
        $alreadySubmitted = !is_null($submissionData);
    @endphp
    ...
    <div class="sus-hero d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
            ...
            <p class="mb-0" style="opacity:0.9;">Share your usability feedback so we can improve MERS.</p>
            <p class="small text-muted mt-2 mb-0"><b> 1 = Strongly Disagree | 2 = Disagree | 3 = Neutral | 4 = Agree |  5 = Strongly Agree.</b></p>
        </div>
    </div>
    ...
    <div class="sus-card">
        ...
        @if ($alreadySubmitted)
            <div class="alert alert-info small">
                You already submitted SUS feedback. The SUS survey only needs to be answered once.
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('recommendations.sus.submit') }}" id="sus-form" novalidate>
            ...
            @for ($i = 1; $i <= 10; $i++)
                @php
                    $defaultAnswer = isset($submissionResponses['q' . $i]) ? (int) $submissionResponses['q' . $i] : null;
                ...
                <div class="sus-question" data-question="sus_q{{ $i }}">
                        ...
                        @for ($score = 1; $score <= 5; $score++)
                            <label class="sus-choice">
                                <input type="radio" name="sus_q{{ $i }}" value="{{ $score }}"
                                    @checked((string) $selectedAnswer === (string) $score)
                    ...
                    <div class="sus-error" id="error-sus_q{{ $i }}">Please select an answer for Question {{ $i }}.</div>
                </div>
            @endfor
    ...
    @unless ($alreadySubmitted)
        <script>
            (function () {
                ...
                const questionNames = Array.from({ length: 10 }, (_, index) => `sus_q${index + 1}`);
                ...
                questionNames.forEach(function (questionName) {
                    ...
                    radios.forEach(function (radio) {
                        radio.addEventListener('change', function () {
                            ...
                            if (formError && form.querySelectorAll('.sus-question.has-error').length === 0) {
